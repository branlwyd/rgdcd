...
fn main() {
    ...
    loop {
        ...
        let current_addr = match current_address(&client) {
            Ok(addr) => addr,
            Err(e) => { error!("Could not get current IP address: {}", e); continue },
        };

        // Update IP in Google Domains if it differs.
        if Some(current_addr) != goog_addr {
            info!("Detected new IP ({} -> {}), updating", fmt_optional_addr(&goog_addr), current_addr);
...
fn current_address(client: &reqwest::Client) -> Result<Ipv4Addr, Box<error::Error>> {
    let mut resp = client.get("https://domains.google.com/checkip")
...
fn update_address(cfg: &Config, client: &reqwest::Client, addr: Ipv4Addr) -> Result<(), Box<error::Error>> {
    let mut resp = client.post("https://domains.google.com/nic/update")
        .header(USER_AGENT, "rgdcd 1.0")
    ...
    Err(format!("update request got error: {} ({})", txt, resp.status()).into())
}

fn update_state(state_path: &Path, state: &State) -> Result<(), Box<error::Error>> {
    let dir = state_path.parent().ok_or_else(|| format!("could not determine parent directory of {}", state_path.display()))?;
